...
if(isset($_POST['delete_bowlingshoes'])){
    ...
    try {
        // Delete from bowlingshoes table
        $deleteStmt = $conn->prepare("DELETE FROM bowlingshoes WHERE ProductID = ?");
        if(!$deleteStmt){
        ...
        if($deleteStmt->affected_rows === 0){
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Bowling shoes not found.'
            ]);
            $deleteStmt->close();
            exit;
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Bowling shoes deleted successfully.',
        ...
        echo json_encode([
            'success' => false,
            'message' => 'An error occurred while deleting the bowling shoes: ' . $e->getMessage()
        ]);
    }
} else {
    http_response_code(400);
