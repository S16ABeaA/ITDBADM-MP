...
if(isset($_POST['delete_bowlingaccessories'])){
    ...
    try {
        // Delete from bowlingaccessories table
        $deleteStmt = $conn->prepare("DELETE FROM bowlingaccessories WHERE ProductID = ?");
        if(!$deleteStmt){
        ...
        if($deleteStmt->affected_rows === 0){
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Bowling accessory not found.'
            ]);
            $deleteStmt->close();
            exit;
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Bowling accessory deleted successfully.',
        ...
        echo json_encode([
            'success' => false,
            'message' => 'An error occurred while deleting the bowling accessory: ' . $e->getMessage()
        ]);
    }
} else {
    http_response_code(400);
