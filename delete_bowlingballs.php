...
if(isset($_POST['delete_bowlingball'])){
    ...
    try {
        // Delete from bowlingball table
        $deleteStmt = $conn->prepare("DELETE FROM bowlingball WHERE ProductID = ?");
        if(!$deleteStmt){
        ...
        if($deleteStmt->affected_rows === 0){
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Bowling ball not found.'
            ]);
            $deleteStmt->close();
            exit;
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Bowling ball deleted successfully.',
        ...
        echo json_encode([
            'success' => false,
            'message' => 'An error occurred while deleting the bowling ball: ' . $e->getMessage()
        ]);
    }
} else {
    http_response_code(400);
